...
class LocallyConnected3D(Layer):
    ...
    ```python
        ...
        # with `data_format="channels_last"`:
        model = Sequential()
        model.add(LocallyConnected2D(64, (3, 3, 3), input_shape=(32, 32, 32, 3)))
        # now model.output_shape == (None, 30, 30, 30, 64)
        # notice that this layer will consume (30*30*30)*(3*3*3*3*64) + (30*30*30)*64 parameters

        # add a 3x3x3 unshared weights convolution on top, with 32 output filters:
        model.add(LocallyConnected2D(32, (3, 3, 3)))
    ...
    # Input shape
        ...
        `(samples, channels, rows, cols, depth)` if data_format='channels_first'
        or 5D tensor with shape:
        `(samples, rows, cols, depth, channels)` if data_format='channels_last'.

    # Output shape
        5D tensor with shape:
        `(samples, filters, new_rows, new_cols, new_depth)` if data_format='channels_first'
        or 5D tensor with shape:
        `(samples, new_rows, new_cols, new_depth, filters)` if data_format='channels_last'.
        `rows` and `cols` values might have changed due to padding.
    """
    ...
    def build(self, input_shape):
        if self.data_format == 'channels_last':
            input_row, input_col, input_dep = input_shape[1:-1]
            input_filter = input_shape[4]
        else:
            input_row, input_col, input_dep = input_shape[2:]
            input_filter = input_shape[1]
        if input_row is None or input_col is None or input_dep is None:
            raise ValueError('The spatial dimensions of the inputs to '
        ...
        output_col = conv_utils.conv_output_length(input_col, self.kernel_size[1],
                                                   self.padding, self.strides[1])
        output_dep = conv_utils.conv_output_length(input_dep, self.kernel_size[2],
                                                   self.padding, self.strides[2])
        self.output_row = output_row
        self.output_col = output_col
        self.output_dep = output_dep
        self.kernel_shape = (output_row * output_col * output_dep,
        ...
        self.kernel = self.add_weight(shape=self.kernel_shape,
                                      ...
                                      constraint=self.kernel_constraint)
        if self.use_bias:
            self.bias = self.add_weight(shape=(output_row, output_col, output_dep, self.filters),
                                        initializer=self.bias_initializer,
                                        name='bias',
                                        regularizer=self.bias_regularizer,
                                        constraint=self.bias_constraint)
    ...
    def compute_output_shape(self, input_shape):
        ...
        cols = conv_utils.conv_output_length(cols, self.kernel_size[1],
                                             self.padding, self.strides[1])
        depth = conv_utils.conv_output_length(depth, self.kernel_size[1],
                                              self.padding, self.strides[1])

        if self.data_format == 'channels_first':
            return (input_shape[0], self.filters, rows, cols, depth)
        elif self.data_format == 'channels_last':
            return (input_shape[0], rows, cols, depth, self.filters)

    def call(self, inputs):
        output = K.local_conv3(inputs,
                               self.kernel,
                               self.kernel_size,
                               self.strides,
                               (self.output_row, self.output_col, self.output_dep),
                               self.data_format)
